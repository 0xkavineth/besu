...
const RETRO_KEY = "lexcase_retro_mode";
...
export default function LexCaseApp({ setPage }) {
  ...
  const toggleRetro = () => {
    setRetro((r) => {
      const next = !r;
      try { localStorage.setItem(RETRO_KEY, next ? "1" : "0"); } catch { /* ignore */ }
      return next;
    });
  };
  ...
  return (
    <div className={retro ? "lexcase-retro" : undefined} style={{ display: "flex", minHeight: "100vh", background: COLORS.bg, position: "relative" }}>
      {mode === "space" && !retro && <SpaceStarfield />}
      {/* Sidebar */}
      <aside style={{
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0, position: "relative", zIndex: 1 }}>
        ...
        <header style={{
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <button
              onClick={toggleRetro}
              title={retro ? t("switchToNormal", lang) : t("switchToRetro", lang)}
              style={{
                display: "flex", alignItems: "center", gap: 6, background: retro ? COLORS.orangeSoft : COLORS.cardAlt,
                border: `1px solid ${COLORS.cardBorder}`, borderRadius: 999, padding: "7px 12px", cursor: "pointer",
                color: retro ? COLORS.orangeDark : COLORS.text, fontSize: "0.78rem", fontWeight: 700,
              }}
            >
              <Landmark size={14} /> {retro ? t("retroModeOn", lang) : t("retroModeOff", lang)}
            </button>
