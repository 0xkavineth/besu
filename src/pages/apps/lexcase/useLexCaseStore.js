...
export function useLexCaseStore(userId) {
  ...
  const upsertCase = useCallback((record) => {
    ...
    setCases((current) => {
      ...
      if (idx === -1) {
        toPersist = { ...record, id: finalId, createdAt: record.createdAt || now, updatedAt: now };
        return [...current, toPersist];
      }
      const prev = current[idx];
      toPersist = { ...record, id: finalId, createdAt: prev.createdAt || record.createdAt || now, updatedAt: now };
      const next = [...current];
      next[idx] = toPersist;
      return next;
    });
  ...
  const addCaseDocument = useCallback((caseId, doc) => {
    ...
    setCases((current) => current.map((c) => {
      ...
      toPersist = { ...c, documents: [...(c.documents || []), doc], updatedAt: now };
      return toPersist;
    }));
    if (userId && toPersist) {
      const { id, ...data } = toPersist;
  ...
  const deleteCaseDocument = useCallback((caseId, docId) => {
    ...
    setCases((current) => current.map((c) => {
      if (c.id !== caseId) return c;
      toPersist = { ...c, documents: (c.documents || []).filter((d) => d.id !== docId), updatedAt: now };
      return toPersist;
    }));
    if (userId && toPersist) {
      const { id, ...data } = toPersist;
